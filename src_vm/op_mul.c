...
static void op_mul_reg_reg(void)
{
  char reg1, reg2;
  uint16_t *p1, *p2;
  ...
  if (core.flagmode.noverbose == 0)
    fprintf(stdout, "%s0x%4.x%s > mul r%d, r%d\n", RED, core.memory.pc, ENDC, reg1, reg2);
...
static void op_mul_reg_dir08(void)
{
  char reg1;
  uint8_t operande;
  uint16_t *p1;
...
static void op_mul_reg_dir16(void)
{
  char reg1;
  uint16_t *p1;
  uint16_t operande;
  ...
  if (core.flagmode.noverbose == 0)
    fprintf(stdout, "%s0x%4.x%s > mull r%d, #0x%x\n", RED, core.memory.pc, ENDC, reg1, operande);
